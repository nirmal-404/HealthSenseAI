...
export default function PatientAppointmentsPage() {
  ...
  const handleDoctorSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      setDoctorError('Doctor search is temporarily unavailable. Use Doctor ID as fallback.');
    } finally {
        ...
        <CardContent className="space-y-4 p-4">
          ...
          {loadingList ? (
            <div className="grid gap-4 xl:grid-cols-2">
              {[0, 1, 2].map((index) => (
                <Card key={`appointment-skeleton-${index}`} className="border border-[#dce5f4] bg-white py-0">
