...
import { Loader2, RefreshCw, AlertCircle, CheckCircle2, Clock, Trash2 } from 'lucide-react';
...
export default function NotificationsPage() {
  const { user } = useAuth();
  const { notifications, loading, fetchNotifications } = useNotifications(user?.userId);
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'appointment':
        return { icon: '📅', label: 'Appointment' };
      case 'payment':
        return { icon: '💳', label: 'Payment' };
      case 'reminder':
        return { icon: '🔔', label: 'Reminder' };
      case 'prescription':
        return { icon: '💊', label: 'Prescription' };
      case 'verification':
        return { icon: '✓', label: 'Verification' };
      case 'message':
    ...
    <div className="space-y-6 p-4 md:p-6 lg:p-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-600 mt-1">
          Manage and view all your notifications and updates
        </p>
      </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className={`flex-shrink-0 rounded-full p-2 ${statusConfig.bgColor}`}>
